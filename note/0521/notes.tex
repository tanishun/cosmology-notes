...
\if0
[11pt,a4paper,dvipdfmx]{jarticle}
\usepackage[height=8.8in,width=6.45in]{geometry}
...
Hypersherical space :
\begin{align}
ds^2 = d\mbx^2 -dz^2,\quad z^2 +\mbx^2 = a^2
\end{align}
Euclidean space :
\begin{align}
ds^2 = d\mbx^2 -dz^2,\quad z^2 - \mbx^2 =a^2\\
...
\if0
\sum_{i} \frac{N_{i} L_{i}}{4 \pi d^{2}}
...
%\footnote{関係詞。}
現在のところ、一部のクエーサーは宇宙の宇宙の電離が完了する前に形成されたと思われており、%\footnote{now appear}、
...
ある赤方偏移$z$に対して、吸収は観測される周波数$\nu_0 = \nu_1/(1+z)$のうちある決まった領域に渡って起こり、その領域は、$t_R$が$t_1$と$t_0$の間にあるという条件と$a(t)$が増加関数であることから求まる。\footnote{$t_1 \leq t_R$から$a(t_1)/a(t_R) \leq 1$で両辺に$\nu_0$をかけたら$\nu_{R} /(1+z) \leq v_{0} $が出て、もう一方は$a(t_R) \leq a(t)$を、直前と同様に逆数とればでる。}
\begin{align}
...
残念ながら、銀河間の空間の中性水素\footnote{21 cm 吸収線を見ている時点で、中性水素を見ていると仮定している。}の温度は、$h \nu_{R} / k_{\mathcal{B}}=0.068\, \mathrm{K}$よりもはるかに大きいために、式\eqref{eq:1.10.6}で与えられる光学的厚さは$0.068 \, \mathrm{K} / \mathrm{T}\left(t_{R}\right)$の因子だけ小さくなってしまう。この吸収線の谷の兆候は未だに発見されていない。
...
調べてみると、例えば、\href{https://arxiv.org/pdf/1809.06661.pdf}{arXiv:1809.06661}によると、$z = 20 − 25$の領域で、14 時間ほど測定すれば、21-cm の　temperature fluctuations のpower spectrum の上限が得られるとのこと。\\
{\mgfamily ここまで。ライマン・アルファ遷移は次にする。}
\if0
現在のところ、銀河間の水素原子の性質は、1s の基底状態から2p の励起状態への遷移であるライマン・アルファ遷移の光子の吸収によって詳しく調べられており、ガーン-ピーターソン効果(Gunn–Peterson effect)として知られている。この効果は、1965年の論文。
吸収の観測は、昔から調べられているようだ。
この遷移は、紫外線領域に共鳴周波数$\nu_{R} = 2.47 \times 10^{15} \, \mathrm{Hz}$を持ち、それは波長1,215 {\AA}に対応する。
しかし、赤方偏移が$z>1.5$にある光源に対しては、式\eqref{eq:1.10.9}から、吸収線の谷の低周波数の極限が波長3,000{\AA}よりも長くなる($(1+z)c/\nu_{R} > 3,036 \,\mathrm{{\AA}}$)。
...
\begin{align}
  \tau_{\nu_{0}=\nu_{R} /(1+z)+}=\left(\frac{n\left(t_{R}\right)}{2.4 h \times 10^{-11} \mathrm{cm}^{-3}}\right)\left(\Omega_{\Lambda}+\Omega_{K}(1+z)^{2}\right.+\Omega_{M}(1+z)^{3}+\Omega_{R}(1+z)^{4} )^{-1 / 2}
\end{align}%
で与えられる。
例えば、もし、宇宙のバリオンの一部$f$が、$z=5$に対応する時刻において、銀河間の中性水素原子にあったとすれば、
$
\Omega_{B} h^{2}=0.02
$
のとき、$z = 5$における水素原子の数密度は$4.8 f \times 10^{5}$である。これはどういう計算をしたらわかるのでしょうか。。。
宇宙論パラメータとして$h=0.65, \Omega_{\Lambda}=0.7, \Omega_{M}=0.3, \, \Omega_{K}=\Omega_{R}=0$を選べば、光学的厚さは、式\eqref{1.10.10}から$4.8 f \times 10^{-5} \, \mathrm{cm}^{-3}$とわかる。
したがって、これらのパラメータに対しては、宇宙のバリオン物質の量に対する比$f \gg 2.6 \times 10^{-6}$である銀河間の中性水素は$z=5$を超える光源からの赤方偏移を受けたライマン・アルファ線より高い周波数を持ついかなる光も完全に吸収してしまうことがわかる。
